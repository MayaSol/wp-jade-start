 ...
 */
function aero_get_svg( $args = array() ) {
...
//  error_log("args = " . $my_arr);
	...
	if ( false === array_key_exists( 'icon', $args ) ) {
		return __( 'Please define an SVG icon filename.', 'kindergarten' );
	}
...
//	write_log($args);
	 ...
	 *
	 * Example 1 with title: <?php echo kindergarten_get_svg( array( 'icon' => 'arrow-right', 'title' => __( 'This is the title', 'textdomain' ) ) ); ?>
	 *
	 * Example 2 with title and description: <?php echo kindergarten_get_svg( array( 'icon' => 'arrow-right', 'title' => __( 'This is the title', 'textdomain' ), 'desc' => __( 'This is the description', 'textdomain' ) ) ); ?>
	 *
	...
	if ( 'social' === $args->theme_location ) {
		foreach ( $social_icons as $attr => $value ) {
			if ( false !== strpos( $item_output, $attr ) ) {
				$item_output = str_replace( $args->link_after, '</span>' . aero_get_svg( array( 'icon' => esc_attr( $value ) ) ), $item_output );
			}
		}
	}
...
add_filter( 'walker_nav_menu_start_el', 'aero_nav_menu_social_icons', 10, 4 );
 ...
 */
function aero_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
...
add_filter( 'nav_menu_item_title', 'aero_dropdown_icon_to_menu_link', 10, 4 );
 ...
 */
function aero_social_links_icons() {
	 ...
	 */
	return apply_filters( 'aero_social_links_icons', $social_links_icons );
}
